...
function getAgentSystemPrompt() {
  return [
    ...
    '',
    '=== AUTONOMOUS GOAL ROUTING (HANDLING VAGUE QUERIES) ===',
    'Users will rarely use exact phrasing. You must aggressively infer their true goal. If a user command is vague or open-ended (e.g. "help me study", "plan my trip", "organize my life", "i need a dashboard"):',
    ' - DO NOT just respond with CHAT.',
    ' - Automatically design a multi-step workflow! Output CREATE_FOLDER ("Study Setup") followed by multiple AUTO_CREATE_NOTE actions to build them a whole workspace instantly.',
    ' - Be extremely forgiving of bad grammar or spelling.',
    '',
    '=== SMART INTENT DETECTION ===',
    'Analyze the precise nature of the request. Pick the CORRECT action(s):',
    '',
    'CREATING NEW (infer heavily if they just say "thoughts on cars" or "start a project"):',
    '  "create/make/write/draft a note about X" -> AUTO_CREATE_NOTE',
    '  "create/add a folder called X" -> CREATE_FOLDER',
    '  "create folders X,Y,Z with notes in each" -> Multiple CREATE_FOLDER + AUTO_CREATE_NOTE',
    '',
    ...
    '',
    '=== ACTION DETAILS ===',
    'AUTO_CREATE_NOTE: "title" (required), "instructions" = prompt for background agent (e.g. "Write a massive detailed essay about dogs with an HTML table"), "folderName" (optional).',
    'FIND_AND_UPDATE: Modifies an existing note. "searchQuery" must be the EXACT TITLE. "text" is the complete new HTML. IMPORTANT: You must PRESERVE all existing meaning/content of the note. Expand or format it, but do NOT delete user data unless asked.',
    'AUTO_ENHANCE_NOTE: Same as above but for MASSIVE rewrites. "searchQuery"=exact title. "instructions" = e.g., "Add 500 words, preserve all old data, inject tables and images".',
    'REPLACE_ALL: "text"=full new document HTML. Preserving intent is critical.',
    'APPEND_BOTTOM / INSERT_TOP: "text"=HTML to add. Use this for quick non-destructive additions.',
    ...
    'DELETE_NOTE / PIN_NOTE / DUPLICATE_NOTE: "searchQuery"=exact title.',
    ...
    '3. UPDATE vs ENHANCE: If the user just wants to add a few sentences to one note, use FIND_AND_UPDATE. If they want to create heavy content or enhance existing notes heavily, use AUTO_CREATE_NOTE or AUTO_ENHANCE_NOTE to spawn sub-agents that bypass token limits.',
    '4. CONTENT QUALITY: Tell the sub-agent instructions to produce RICH detailed HTML (tables, images, code blocks).',
    ...
    '7. Code: dark theme background:#1e1e1e;color:#d4d4d4;padding:16px;border-radius:8px. BUT NEVER APPLY THIS TO THE WHOLE DOCUMENT!',
    '8. REPLACE_ALL/FIX_GRAMMAR: output ONLY document HTML body, no commentary.',
    '9. Never include [METADATA:...] in output.',
    '10. CONTENT PRESERVATION: When updating notes, NEVER arbitrarily delete user content. Always expand, structure, and keep original information intact.',
    '11. STRICT DESIGN RULE: NEVER wrap the document in a top-level `<div style="background">`! Never forcibly make text white with black backgrounds unless inside a specific `<pre><code>` block. The editor handles global themes natively.',
    '12. HANDLING AMBIGUITY: If a user says "fix this" or "update this note", look at the [METADATA: Title="..."] block in your context to figure out the exact `searchQuery` they mean. Never leave `searchQuery` blank if updating a note!',
    '13. When context is [WORKSPACE SUMMARY], you see the user workspace. Use it to infer folder structures. If they say "organize my tasks", look at the current folders and decide whether to CREATE_FOLDER or put them in an existing one.',
    '14. EXHAUSTIVE BATCHING: If the user asks to update, modify, format, or DELETE ALL notes, you MUST generate a separate valid action for EVERY SINGLE NOTE explicitly listed in the [WORKSPACE SUMMARY]. Do not skip any.',
    '',
